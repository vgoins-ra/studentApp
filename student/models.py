...
#Create School Table with relationship to Student_Records Table (field=school) 
...
# Create Faculty Table with relationship to School Table (foreign key=school_assigned)
...
#Create Department Table with relationship to Faculty Table (foreign key=faculty)
...
#Create Grade Table with relationship to Student_Records Table (related=grade_id)
...
# Create Student_Record Table which relationship to many tables foreign keys=department,grade,school
# additional dropdown value fields = year_of_graduation 
...
#Create Certificate Table with relationship to Student_Certificates Table (dropdown value= certificates)
...
#Create Certificates Table with relationship to Certificate_Type Table (foreign keys=name and certificate)
#students can have multiple certificates
class Certificates(models.Model):
    
    name = models.ForeignKey(Student_Record,related_name='certif_student', max_length = 100, null=True,on_delete=models.CASCADE)
    certificate = models.ForeignKey(Certificate_Type,max_length = 100,on_delete=models.CASCADE)

    def __str__(self):
        return str(self.certificate_id) or ''
